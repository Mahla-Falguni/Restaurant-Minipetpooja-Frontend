...
Same flow as the restaurant-side ForgotPassword.jsx, pointed at
the super admin axios instance / SuperAdmin collection instead.
Kept as a separate page (not shared) since super admin auth is
an entirely separate realm from restaurant staff auth.
=========================================================
*/
...
const SuperAdminForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await superAdminAxiosInstance.post("/auth/forgot-password", { email: email.trim() });
      setSent(true);
    } catch (error) {
  ...
  return (
    <AuthLayout
      ...
      subtitle={
        sent
          ? "If a platform admin account exists for that address, a reset link is on its way."
